...
# without the mirror keep the original public install path. The default
# toolchain is read from vllm's rust-toolchain.toml (falling back to stable),
# so it always matches the version vllm pins.
RUN if [ -n "$RUSTUP_DIST_SERVER" ]; then \
      ...
      wget -q "${RUSTUP_UPDATE_ROOT}/dist/${ARCH}/rustup-init" -O /tmp/rustup-init && \
      chmod +x /tmp/rustup-init && \
      /tmp/rustup-init -y --default-toolchain "${TOOLCHAIN:-stable}" && \
      rm /tmp/rustup-init && \
      . "$HOME/.cargo/env"; \
    fi
