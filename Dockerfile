...
ARG MOONCAKE_INDEX_URL="https://repo.huaweicloud.com/repository/pypi/simple"
ARG PYTORCH_INDEX_URL="https://repo.huaweicloud.com/repository/pypi/simple"
...
RUN if [ -n "$APTMIRROR" ]; then \
        sed -Ei "s@(ports|archive).ubuntu.com@${APTMIRROR#*://}@g" /etc/apt/sources.list; \
    fi && \
    apt-get update -y && \
