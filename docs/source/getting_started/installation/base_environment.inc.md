=== "Build from a base environment"

    #### Build from a base environment {: #installation-base-environment }
    ...
    ???+ note
        ...
        For a non-default installation path, source the corresponding `set_env.sh`. Also refer to the [CANN installation resources](https://www.hiascend.com/cann/download) to complete the installation.
    ...
    ???+ important "Verify NNAL separately"

        The vLLM Ascend runtime requires NNAL (Ascend Neural Network Acceleration Library). The NNAL package is typically named `Ascend-cann-nnal_...run`.
