...
def generate_md(model_name, tasks_list, args, datasets):
  ...
  preamble = f"""<div>
    ...
    vllm: {args.vllm_version} <br> 
    vllm-ascend: {args.vllm_ascend_version} <br>
    torch: {args.torch_version} <br> 
    torch_npu: {args.torch_npu_version} <br> 
    cann: {args.cann_version} <br>
</div>
<div>
...
def main(args):
    ...
    if not is_multimodal(args.model):
