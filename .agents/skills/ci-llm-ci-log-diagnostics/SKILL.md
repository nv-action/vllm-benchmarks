...
description: Two-phase CI log shrinking for LLM failure diagnosis (vllm-benchmarks GitHub Actions). Use when wiring or debugging optional LLM diagnostics after test failures.
---
...
- **Master switch**: `VLLM_ASCEND_CI_LLM_ENABLED` (environment). **Default `0`**: no log read, no HTTP; workflows keep the step so code can merge before wiring a model. Set to **`1`** and configure key/base to call the API.
