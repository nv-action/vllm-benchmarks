...
# Get the failure block for the failed test
python3 .github/workflows/scripts/ci_diagnosis_evidence.py failure-block \
  --log-file /tmp/leader_logs.txt --test tests/e2e/test_inference.py::test_basic

# Get K8s pod state
python3 .github/workflows/scripts/ci_diagnosis_evidence.py k8s-summary \
  --artifact-dir /tmp/ci-diagnosis-artifacts
```
