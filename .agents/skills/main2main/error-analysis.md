...
### 1.1 Local Log File Path Entry

If the caller provides a local CI log file path, run the repository summary script from the repository root:

```bash
python3 <VLLM_ASCEND_DIR>/.github/workflows/scripts/ci_log_summary.py \
  --log-file <LOG_FILE_PATH> \
  --format llm-json \
  --output /tmp/main2main-failure-summary.json
```


### 1.3 Run ID Entry

If the caller provides a run ID, run the repository summary script from the
repository root using that run ID:
...
Load `/tmp/main2main-failure-summary.json` and extract the key fields:
...
**Phase 1 outputs:** `GOOD_COMMIT`, `BAD_COMMIT`, `failed_test_cases`,
`code_bugs`, and `env_flakes`
...
```markdown
### CI Fix Summary (run ID: <RUN_ID> if available)
