...
run_suite.py fails. This script streams run_suite.py output to both stdout and
the step log, runs ci_log_summary.py on that log, and records a machine-readable
result JSON.
...
def _run_and_tee(command: list[str], cwd: Path, log_path: Path, env: dict[str, str]) -> int:
...
def main() -> None:
    ...
    parser.add_argument("--suite", default="e2e-main2main",
                        help="run_suite.py suite name")
    ...
    run_suite_exit_code = _run_and_tee(command, ascend_path, log_path, env)
