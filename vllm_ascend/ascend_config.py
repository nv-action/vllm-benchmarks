...
class AscendConfig:
    ...
    def update_compile_ranges_endpoints(self):
        ...
        if self.ascend_compilation_config.enable_npugraph_ex:
            if self.ascend_compilation_config.fuse_allreduce_rms:
                ...
                new_compile_ranges_endpoints = vllm_config.compilation_config.compile_ranges_endpoints
                new_compile_ranges_endpoints.append(ALLREDUCE_NORM_FUSE_THRESHOLD)
                new_compile_ranges_endpoints = sorted(new_compile_ranges_endpoints)
                vllm_config.compilation_config.compile_ranges_endpoints = new_compile_ranges_endpoints
                logger.debug(
                    "set compile_ranges_endpoints to "
                    "{new_compile_ranges_endpoints} for matmul and allreduce fusion"
                )
            ...
            if vllm_config.compilation_config.pass_config.enable_sp and not is_moe_model(vllm_config):
                ...
                new_compile_ranges_endpoints.append(sp_threshold)
                logger.debug(f"add {sp_threshold} to compile_ranges_endpoints for sequence parallelism")
            if len(new_compile_ranges_endpoints) > len(vllm_config.compilation_config.compile_ranges_endpoints):
                new_compile_ranges_endpoints = sorted(new_compile_ranges_endpoints)
                vllm_config.compilation_config.compile_ranges_endpoints = new_compile_ranges_endpoints
