...
class POFileManager:
    ...
    def __init__(self, po_dir: str = "docs/locale/zh_CN/LC_MESSAGES", api_key: Optional[str] = None):
        ...
        if not api_key:
            print("❌ 未找到 DeepSeek API 密钥")
            print("请设置环境变量 DEEPSEEK_API_KEY 或在代码中提供 api_key")
            sys.exit(1)
    ...
    def _translate_single(self, po_path: str, content: str) -> bool:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "你是一位专业的技术文档翻译专家，精通中英文技术文档翻译。"},
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PO File Translation Manager - Translate specified PO files (DeepSeek version)'
    )
    parser.add_argument(
        '--detect-only',
        action='store_true',
    ...
    file_list = None
    if args.files:
        file_list = [f.strip() for f in args.files.split(',')][1:2]

    print("🚀 Starting PO File Translation Manager (DeepSeek version)")
    ...
    manager = POFileManager(po_dir, api_key=api_key)
