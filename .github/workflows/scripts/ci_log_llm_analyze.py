"""
POST failure logs to an OpenAI-compatible chat API (CI only, optional).

Reads ``VLLM_ASCEND_CI_LLM_*`` from the environment (same names as vllm-ascend CI
so one org secret set can be reused). Optionally uses ``vllm_ascend.envs`` when
that package is importable on PYTHONPATH.
...
def _ci_llm_config_from_env() -> dict[str, str | int]:
    """Defaults when vllm_ascend.envs is not available."""
    return {
...
def main() -> int:
    ...
    if not api_key or not base_url:
        _append_step_summary(
            ...
            "`https://api.openai.com/v1`), and enable with variable "
            "`VLLM_ASCEND_CI_LLM_ENABLED=1`._\n"
        )
        return 0
    ...
    system_prompt = (
        "You are a senior CI engineer for the nv-action/vllm-benchmarks project "
        "(vLLM on Huawei Ascend NPU, nightly and E2E workflows). "
