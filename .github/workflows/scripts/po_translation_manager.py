...
class POTranslator:
    ...
    def _translate_in_chunks(self, po_path: str, content: str) -> bool:
        """Translate large file in chunks"""
        lines = content.split('\n')
        chunk_size = 100
...
def main():
    ...
    file_list = [f.strip() for f in args.files.split(',') if f.strip()][1:2]
    ...
    # Generate report
    report = translator.generate_report(translated_files, [Path(f).name for f in failed_files])
    print(report)
